...
当前版本是一款 Android、iPhone 和 iPad 基础兼容的中俄双向实时语音翻译 App。主持人创建独立会议，任意数量的注册用户和临时用户可通过 App 内好友邀请、二维码、邀请链接或房间码加入。所有人按住说话、松开提交，服务端完成识别、翻译和目标语言语音合成，各设备同步显示发言者、原文、译文和状态。
...
不包含：连续同声传译、多人同时抢占同一麦克风通道、视频通话、Viewer、完整 CRM、企业级 SSO/组织后台、长期录音、语音克隆和付费系统。当前会议纪要提供可审计的结构化数据与确定性草稿；生产级大模型摘要质量仍需单独接入和验收。
...
客户端永远不直连阿里云语音服务，避免泄露 API Key，也保证术语、限流、审计与客户隔离统一执行。原始需求优先考虑 Gummy，但截至 2026-07-18，官方模型目录已把 Gummy 列为即将下线能力，且不同官方页面的中俄方向说明存在差异；新生产接入不能以 Gummy 为唯一链路。当前代码没有 Gummy adapter 或开关；若业务坚持评估，必须先实现服务端适配器，并在目标账号、地域和真实双向音频上验收。
...
5. WebSocket 广播 `translation.processing`。
...
8. 服务端用状态条件更新同一条 `PROCESSING` 消息，保存最终原文、译文、音频引用或 `TTS_FAILED`，不再分配新 `sequence`。
9. 广播 `translation.final`；音频成功时带内部签名 URL，失败时 `audioUrl=null` 并带 `TTS_FAILED`。当前没有异步 `translation.audio.ready` 事件或独立 TTS 重试端点。
10. 客户端按 `messageId` 去重、按 `sequence` 排序，并根据自动播放设置入队。
...
当前 `ConversationSummary` 已保存参会人员、核心讨论、各方观点、确认事项、待办和未解决问题，并通过指定 `conversationId` 读取消息。只有会议结束后，主持人才可明确生成或更新；普通查看严格使用 GET，不会重写纪要。每份纪要记录来源最大序号、来源消息数、消息最后更新时间和 revision；因此确认纠错即使不新增消息也会使旧纪要过期。旧版无可靠来源边界时标记为“无法验证”，不能伪装成最新。任何未来的客户长期总结必须由用户显式选定 `contactId` 后合并，不允许按日期或当前界面上下文隐式跨会议取数。生产级生成式摘要需要另行接入、评测和人工复核，且不得阻塞实时翻译链路。

会议结束且纪要来源未过期时，会议 owner 可以选择有权限且有邮箱的 Participant 邮件分发纪要。注册用户邮箱来自 User，临时用户在加入时填写并写入 GuestIdentity/Participant 会议快照；客户端只得到脱敏邮箱提示。`SummaryEmailDistribution` 与 `SummaryEmailRecipient` 是 PostgreSQL 持久任务/逐收件人 claim，保存纪要 revision、请求幂等摘要和供应商受理结果。API 快速返回，后台 worker 可在进程重启后继续扫描；多副本通过收件人 CAS 避免同时发送。邮件逐人发送，不使用群发 To/CC；worker 在任务开始和每封发送前重验纪要来源和参会权限。生产使用 Resend，API Key 只存在服务端 Secret Manager，发信域名必须验证 SPF/DKIM。

## 10.1 服务器运营面

`apps/admin-web` 是与 API 同源交付的轻量管理界面，不引入第二套认证或客户端权限源。它复用正式账号登录，但所有管理 API 都额外从 PostgreSQL 重新验证 `isSystemAdmin`/不可复用 User ID 引导白名单；未验证且可重新注册的邮箱永远不是权限依据。该运营面只默认显示会议元数据、参会人员和消息计数，不把全量会议正文变成默认后台浏览面。用户停用、会话撤销、一次性密码重置和强制结束会议均有事务内审计记录。
...
- `ConversationSummary` 保存 `participantRoster`、`coreDiscussion`、`partyViews`、`confirmedItems`、`actionItems` 和 `openQuestions`；核心讨论逐条保留 participantId 和发言者快照，并用 `sourceMaxSequence`、`sourceMessageCount`、`sourceLatestMessageUpdatedAt`、`revision` 标识来源版本。
