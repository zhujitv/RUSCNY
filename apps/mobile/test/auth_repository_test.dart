...
void main() {
  ...
  test('registers the native push identifier on the authenticated device',
      () async {
    final tokenStore = SecureTokenStore();
        ...
        onRequest: (options, handler) {
          ...
          handler.resolve(
            Response<dynamic>(
              ...
              data: {
                'ok': true,
                'data': {'registered': true, 'deliveryEnabled': true},
              },
            ),
          );
        },
    ...
    final deliveryEnabled = await repository.registerPushNotification(
      'fcm-registration-id',
      bindingId: '06927bcd-9b16-4480-a031-33fbb4a84732',
    );

    expect(deliveryEnabled, isTrue);
    expect(request.method, 'POST');
    expect(request.path, endsWith('/auth/push-registration'));
    expect(request.data, {
      'provider': 'FCM',
      'registrationId': 'fcm-registration-id',
      'bindingId': '06927bcd-9b16-4480-a031-33fbb4a84732',
    });
  });
}
